...
def generate_accounts_background(user_id, region, count, name_prefix, password_prefix, chat_id, progress_msg_id):
    ...
    try:
        # Reset counter global untuk sesi ini (Pake LOCK biar aman)
        with LOCK:
            SUCCESS_COUNTER = 0
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            futures = [
                executor.submit(
                    ...
                    region, name_prefix, password_prefix, count, i + 1, is_ghost
                ) for i in range(count)
            ]
            
            # Pantau setiap thread yang selesai secara real-time
            for future in as_completed(futures):
                if EXIT_FLAG:
                    # Jika ada sinyal stop, batalkan sisa thread
                ...
                try:
                    ...
                    # Update progress ke Telegram tiap kelipatan 5 atau saat finish
                    # Biar gak kena limit "Too Many Requests" dari API Telegram
                    if success_count % 5 == 0 or success_count == count:
                        with LOCK:
                            # Ambil data terbaru dari variabel global
                            current_rare = RARE_COUNTER
                            current_couples = COUPLES_COUNTER
                        
                        progress_text = (
                            f"🚀 **BISHAL Multi-Thread Gen**\n"
                            f"━━━━━━━━━━━━━━━━━━\n"
                            f"📊 **Status:** Running (5 Threads)\n"
                            f"✅ Generated: **{success_count}/{count}**\n"
                            f"💎 Rare Found: **{current_rare}**\n"
                            f"💑 Couples: **{current_couples}**\n"
                            f"🌍 Region: **{region}**\n"
                            f"━━━━━━━━━━━━━━━━━━\n"
                            f"⏳ *Processing parallel... please wait.*"
                        )
                        ...
                        try:
                            ...
                            # Skip jika konten pesan sama (Telegram Error)
                            pass

                except Exception as thread_err:
                    print(f"⚠️ Error in worker thread: {thread_err}")

        # --- FINAL REPORT SETELAH SEMUA THREAD BERES ---
        with LOCK:
            final_rare = RARE_COUNTER
            final_couples = COUPLES_COUNTER

        final_text = f"""
🎉 **Generation Complete!**

📊 **Final Statistics:**
✅ Total Success: **{success_count}/{count}**
💎 Rare Accounts: **{final_rare}**
💑 Couples Pairs: **{final_couples}**
🌍 Region: **{region}**

📁 Semua akun telah disimpan ke folder database.
Silakan gunakan command /download untuk mengambil file.
        """
        ...
        print(error_msg)
        try:
            bot.edit_message_text(error_msg, chat_id, progress_msg_id)
        except:
